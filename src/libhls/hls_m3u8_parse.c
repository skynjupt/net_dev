...
static int hls_parse_m3u_stream(memory_stream_info *stream, hls_ts_node_t *p_ts_list, char *source_url)
{
	char *temp1,*temp2;
	char tempstr[HLS_MAX_URL_LEN];
	UINT8 e_flag = 0;
					...
					if(temp1)
					{
						...
						if(temp2)
						{
							*temp2 = '\0';
							...
							HLS_PRINTF("duration=%d\n",p_ts_list[ts_num].section_duration);
							e_flag = 1;
						}
					}
				}
...
//						hls_get_key_tag_info(temp1, &(info->encypt_info));
			...
			else if(strlen(tempstr)>3) //URL
			{
				if(e_flag == 1)
				{
					if(0 == hls_merge_url(source_url, tempstr, &p_ts_list[ts_num].url))
					{
